...
type TimelineAppProps = {
    isOpen: boolean;
    onOpen: () => void;
};

export default function TimelineApp({ isOpen, onOpen }: TimelineAppProps) {
    return (
        <button
            type="button"
            ...
            onClick={onOpen}
            className="group fixed left-[2.25rem] top-[24.6rem] z-[24] grid w-[clamp(7.8rem,16vw,11.2rem)] cursor-pointer content-start justify-items-center gap-[0.8rem] border border-transparent bg-transparent px-[0.42rem] py-[0.63rem] transition-colors duration-100 hover:border-[#cfd5df]/55 hover:bg-[#b7bcc8]/28 focus-visible:border-[#d5dbe5]/65 focus-visible:bg-[#b7bcc8]/32 max-[760px]:top-[21.35rem]"
        >
            <span
